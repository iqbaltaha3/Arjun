...
from .palette import (
    CHARCOAL, CHART_COLORWAY, FONT_STACK, MUTED, WHITE,
)
...
def to_plotly(chart: Chart):
    ...
    if ctype == "pie":
        fig.add_trace(
            go.Pie(
                ...
                marker=dict(line=dict(color=CHARCOAL, width=2)),
            )
        )
    elif ctype == "line":
        for s in series:
    ...
    fig.update_layout(
        title=dict(text=chart.get("title", ""), font=dict(size=16, color=WHITE)),
        template="plotly_dark",
        paper_bgcolor=CHARCOAL,
        plot_bgcolor=CHARCOAL,
        colorway=CHART_COLORWAY,
        font=dict(family=FONT_STACK, color=WHITE, size=13),
        height=430,
...
def to_matplotlib(chart: Chart):
    ...
    fig = Figure(figsize=(8, 4.6), dpi=110, layout="constrained", facecolor=CHARCOAL)
    ax = fig.subplots()
    ax.set_facecolor(CHARCOAL)
    ax.set_prop_cycle(color=CHART_COLORWAY)
    ax.set_title(chart.get("title", ""), fontsize=12, fontweight="bold", color=WHITE)
    ...
    if ctype == "pie":
        ax.pie(
            ...
            startangle=90,
            counterclock=False,
            wedgeprops={"linewidth": 1, "edgecolor": CHARCOAL},
            colors=CHART_COLORWAY * 4,
    ...
    if ctype == "line":
        ...
        for i, s in enumerate(series):
            ...
            ax.bar_label(bars, labels=_labels(s["values"]), fontsize=8, padding=2, color=WHITE)
